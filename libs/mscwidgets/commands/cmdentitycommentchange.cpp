...
CmdEntityCommentChange::CmdEntityCommentChange(MscEntity *item, const QString &newComment)
    : BaseCommand(item)
    , m_oldComment(item->comment())
...
{
    if (m_modelItem)
        m_modelItem->setComment(m_newComment);
}

void CmdEntityCommentChange::undo()
{
    if (m_modelItem)
        m_modelItem->setComment(m_oldComment);
}
