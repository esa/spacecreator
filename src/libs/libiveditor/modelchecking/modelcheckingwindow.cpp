...
void ModelCheckingWindow::addProperty()
{
    ...
    QStringList propertyTypes = { "Boolean Stop Condition - LTL", "Boolean Stop Condition - Observer",
        "Message Sequence Chart Search/Verify", "Observer" };
    ...
    if (ok1 && !propertyType.isEmpty()) {
        ...
        if (propertyType == "Message Sequence Chart Search/Verify") {
            makeRule = "create-msc";
        }
        if (propertyType == "Boolean Stop Condition - Observer") {
            makeRule = "create-bsc";
        }
        if (propertyType == "Boolean Stop Condition - LTL") {
            QMessageBox::warning(this, tr("Add property"), "LTL not yet supported.");
            return;
        }
        bool ok2;
        ...
        QString propertyName =
                QInputDialog::getText(this, "New " + propertyType, label, QLineEdit::Normal, "new_property", &ok2);
        ...
        if (ok2 && !propertyName.isEmpty()) {
            ...
            if (makeCallerProcess->execute(makeCall)) {
                QMessageBox::warning(this, tr("Add property"), tr("Error executing '%1'").arg(makeCall));
            }
            ...
            this->propertiesTopDirWidgetItem->setCheckState(0,
                    listProperties(
                            this->propertiesTopDirWidgetItem, propertiesFileInfo, preSelection, expandedNodes, 0));
