...
SymbolDefinitionReader::ExternalReferencesMap SedsXmlImporter::readExternalReferences(const Options &options) const
{
    ...
    if (const auto externalRefFilename = options.value(SedsOptions::externalRefFilename); externalRefFilename) {
        externalReferences = SymbolDefinitionReader::readSymbols(*externalRefFilename);
    }
...
{
    const auto inputFilename = options.value(SedsOptions::inputFilename);
    if (!inputFilename) {
    ...
    auto preprocessedFilename = [&]() {
        const auto value = options.value(SedsOptions::preprocessedFilename);
        if (value) {
            return *value;
        } else {
...
void SedsXmlImporter::validate(const QString &preprocessedFilename, const Options &options) const
{
    const auto schemaFilename = [&]() {
        const auto value = options.value(SedsOptions::schemaFilename);
        if (value) {
            return *value;
        } else {
