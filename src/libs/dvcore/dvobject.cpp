...
struct DVObjectPrivate {
    DVObjectPrivate(const shared::Id &id, const DVObject::Type t)
...
DVObject::DVObject(const DVObject::Type t, const QString &title, QObject *parent, const shared::Id &id)
    : QObject(parent)
    , d(new DVObjectPrivate(id, t))
{
    connect(this, qOverload<const QString &>(&DVObject::propertyChanged), this,
            [this](const QString &token) { Q_EMIT propertyChanged(meta::Props::token(token)); });

    connect(this, qOverload<const QString &>(&DVObject::attributeChanged), this,
...
shared::Id DVObject::id() const
{
    return d->m_id;
}

DVObject::Type DVObject::deployType() const
{
    return d->m_type;
}
...
QVector<qint32> DVObject::coordinatesFromString(const QString &strCoordinates)
{
...
QString DVObject::coordinatesToString(const QVector<qint32> &coordinates)
{
    QString coordString;
...
QHash<QString, QVariant> DVObject::attrs() const
{
    return d->m_attrs;
}

void DVObject::setAttrs(const QHash<QString, QVariant> &attrs)
{
    if (d->m_attrs != attrs) {
        d->m_attrs.clear();
...
QVariant DVObject::attr(const QString &name, const QVariant &defaultValue) const
{
    return d->m_attrs.value(name, defaultValue);
}

void DVObject::setAttr(const QString &name, const QVariant &val)
{
    d->m_attrs[name] = val;
    Q_EMIT attributeChanged(name);
}

void DVObject::removeAttr(const QString &name)
{
 ...
 */
bool DVObject::hasAttribute(const QString &attributeName, const QVariant &value) const
{
 ...
 */
bool DVObject::hasAttributes(const QHash<QString, QVariant> &attrs) const
{
...
QHash<QString, QVariant> DVObject::props() const
{
    return d->m_props;
}

void DVObject::setProps(const QHash<QString, QVariant> &props)
{
    if (props != d->m_props) {
        d->m_props.clear();
...
QVariant DVObject::prop(const QString &name, const QVariant &defaultValue) const
{
    return d->m_props.value(name, defaultValue);
}

void DVObject::setProp(const QString &name, const QVariant &val)
{
...
void DVObject::removeProp(const QString &name)
{
 ...
 */
bool DVObject::hasProperty(const QString &propertyName, const QVariant &value) const
{
 ...
 */
bool DVObject::hasProperties(const QHash<QString, QVariant> &props) const
{
...
void DVObject::setObjectsModel(DVModel *model)
{
    d->m_model = model;
}

DVModel *DVObject::objectsModel() const
{
    return d->m_model;
}
