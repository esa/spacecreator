...
void
Options::add(Options::Key key)
{
    m_options.insert({ key, std::nullopt });
}

void
Options::add(Options::Key key, QString value)
{
    m_options.insert({ key, std::move(value) });
}

bool
Options::isSet(Options::Key key) const
{
...
Options::value(Options::Key key) const
{
    const auto found = m_options.find(key);
    if(found != std::end(m_options)) {
        return found->second;
...
std::vector<QString>
Options::values(Options::Key key) const
{
