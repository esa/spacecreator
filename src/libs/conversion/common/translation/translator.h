...
ModelT *Translator::getModel(const std::vector<Model *> models)
{
    ...
    if (foundModels.empty()) {
        auto message = QString("Missing source %1 model").arg(modelTypeToString(ModelProperties<ModelT>::type));
        throw conversion::translator::TranslationException(std::move(message));
    } else if (foundModels.size() > 1) {
        auto message =
                QString("More than 1 source %1 model passed").arg(modelTypeToString(ModelProperties<ModelT>::type));
        throw conversion::translator::TranslationException(std::move(message));
    } else {
