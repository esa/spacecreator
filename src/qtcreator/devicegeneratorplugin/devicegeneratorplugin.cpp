...
bool DeviceGeneratorPlugin::initialize(const QStringList &arguments, QString *errorString)
{
    ...
    auto action = new QAction(tr("DeviceGenerator Action"), this);
    Core::Command *cmd = Core::ActionManager::registerAction(action, Constants::ACTION_ID,
                                                             Core::Context(Core::Constants::C_GLOBAL));
    cmd->setDefaultKeySequence(QKeySequence(tr("Ctrl+Alt+Meta+A")));
    connect(action, &QAction::triggered, this, &DeviceGeneratorPlugin::triggerAction);

    Core::ActionContainer *menu = Core::ActionManager::createMenu(Constants::MENU_ID);
    menu->menu()->setTitle(tr("DeviceGenerator"));
    menu->addAction(cmd);
...
void DeviceGeneratorPlugin::triggerAction()
{
    QMessageBox::information(Core::ICore::mainWindow(),
                             tr("Action Triggered"),
                             tr("This is an action from DeviceGenerator."));
}
