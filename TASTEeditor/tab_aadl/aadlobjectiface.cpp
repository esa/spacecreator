...
AADLObjectIface::AADLObjectIface(AADLObject::Type ifaceType, const QString &title, AADLObject *parent,
                                 const common::Id &id)
    : AADLObject(ifaceType, title, parent, id)
...
AADLObjectIfaceProvided::AADLObjectIfaceProvided(AADLObject *parent)
    : AADLObjectIface(AADLObject::Type::ProvidedInterface, tr("PI_%1").arg(++sProvidedCounter), parent)
{
}

AADLObjectIfaceProvided::AADLObjectIfaceProvided(const QString &title, AADLObject *parent)
    : AADLObjectIface(AADLObject::Type::ProvidedInterface, title, parent)
{
}

AADLObjectIfaceProvided::AADLObjectIfaceProvided(const common::Id &id, const QString &title, AADLObject *parent)
    : AADLObjectIface(AADLObject::Type::ProvidedInterface, title, parent, id)
{
}

AADLObjectIfaceRequired::AADLObjectIfaceRequired(AADLObject *parent)
    : AADLObjectIface(AADLObject::Type::RequiredInterface, tr("RI_%1").arg(++sRequiredCounter), parent)
{
    setProp(meta::Props::token(meta::Props::Token::Autonamed), isAutogeneratedName());
}

AADLObjectIfaceRequired::AADLObjectIfaceRequired(const QString &title, AADLObject *parent)
    : AADLObjectIface(AADLObject::Type::RequiredInterface, title, parent)
{
    setProp(meta::Props::token(meta::Props::Token::Autonamed), isAutogeneratedName());
}

AADLObjectIfaceRequired::AADLObjectIfaceRequired(const common::Id &id, const QString &title, AADLObject *parent)
    : AADLObjectIface(AADLObject::Type::RequiredInterface, title, parent, id)
{
    setProp(meta::Props::token(meta::Props::Token::Autonamed), isAutogeneratedName());
}
...
bool AADLObjectIface::isAutogeneratedName() const
{
    return isAutogeneratedName(title());
}

bool AADLObjectIface::isAutogeneratedName(const QString &name)
{
    if (name.isEmpty())
        return false;

    static const QString preffixPI(tr("PI_"));
    static const QString preffixRI(tr("RI_"));

    if (!name.startsWith(preffixRI) && !name.startsWith(preffixRI))
        return false;

    if (name == preffixRI)
        return false;

    for (int i = preffixPI.length(); i < name.length() - 1; ++i)
...
void AADLObjectIfaceRequired::setAttr(const QString &name, const QVariant &val)
{
    if (!name.isEmpty()) {
        ...
        case meta::Props::Token::name: {
            const bool autoName = isAutogeneratedName(val.toString());
            ...
            if (m_originalFields.collected()) {
                ...
                if (!autoName)
                    m_originalFields.attrs[name] = val;
            } else {
...
QStringList AADLObjectIfaceRequired::inheritedLables() const
{
    ...
    else if (isAutogeneratedName() && isInheritPI()) {
