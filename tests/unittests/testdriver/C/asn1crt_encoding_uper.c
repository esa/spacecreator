...
static flag ObjectIdentifier_subidentifiers_uper_decode(
        BitStream *pBitStrm, asn1SccSint *pRemainingOctets, asn1SccUint *siValue)
{
    byte curByte;
...
flag ObjectIdentifier_uper_decode(BitStream *pBitStrm, Asn1ObjectIdentifier *pVal)
{
    ...
    // return true, if totalSize reduced to zero. Otherwise we exit the loop because more components we present than
...
flag RelativeOID_uper_decode(BitStream *pBitStrm, Asn1ObjectIdentifier *pVal)
{
    ...
    // return true, if totalSize is zero. Otherwise we exit the loop because more components were present than
    // OBJECT_IDENTIFIER_MAX_LENGTH
    return totalSize == 0;
}
