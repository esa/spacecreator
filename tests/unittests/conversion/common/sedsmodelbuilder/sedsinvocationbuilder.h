...
class SedsInvocationBuilder final
{
public:
    SedsInvocationBuilder(QString name);

    auto build() -> seds::model::ActivityInvocation;

    auto withArgument(const QString name, const QString value) -> SedsInvocationBuilder &;

private:
    seds::model::ActivityInvocation m_invocation;
};
