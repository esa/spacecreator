/* Body file for GUI Clock
 * Generated by TASTE/Kazoo on 2022-05-16 12:08:08
...
static mqd_t clock_RI_queue_id;
...
void clock_startup(void)
{
   ...
   if (msgsize_max < sizeof (PI_Messages) || msgsize_max < sizeof (RI_Messages)) {
    ...
    len = snprintf (gui_queue_name, 0, "%d_clock_RI_queue", geteuid());
    gui_queue_name = (char *) malloc ((size_t) len + 1);
    if (NULL != gui_queue_name) {
       snprintf (gui_queue_name, len + 1, "%d_clock_RI_queue", geteuid());

       create_exchange_queue(gui_queue_name, 5, sizeof(RI_Messages), &clock_RI_queue_id);
...
void clock_PI_Poll(void)
{
   ...
   T_clock_RI_list message_received_type;
   ...
   mq_getattr(clock_RI_queue_id, &msgq_attr);

   while (!retrieve_message_from_queue(clock_RI_queue_id,
                                       sizeof(RI_Messages),
                                       msgcontent,
                                       (int *)&message_received_type)) {
      switch(message_received_type) {
         case i_ClockInterface:
            clock_RI_ClockInterface();
            break;
       default : break;
      }
   }
