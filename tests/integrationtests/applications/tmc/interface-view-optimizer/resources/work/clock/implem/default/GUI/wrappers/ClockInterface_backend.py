...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_ClockInterface
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_clock_RI_ClockInterface
    tcId = i_clock_RI_ClockInterface
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global ClockInterface_via_shared_lib
    shared_lib = True
    ClockInterface_via_shared_lib = dll.clock_PI_ClockInterface
...
def send_ClockInterface_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_ClockInterface(tc)
            except IOError:
                raise
    elif udp:
...
def fromPysideToASN1(_):
    ...
    # ClockInterface = ASN1.T_Null_Record()
    return None
