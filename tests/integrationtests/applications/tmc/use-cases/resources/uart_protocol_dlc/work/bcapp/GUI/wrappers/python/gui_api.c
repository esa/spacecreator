...
T_bcapp_RI_list ii_DLCTxReset = i_DLCTxReset;
typedef struct {
    int tc_id;
...
int SendTC_DLCTxReset(void *p_no_param)
{
    ...
    if (((mqd_t)-2) == q) {
        ...
        sprintf(QName, "%d_bcapp_RI_queue", geteuid());
        open_exchange_queue_for_writing(QName, &q);
    }
    DLCTxReset_TCDATA data;
    data.tc_id = (int) i_DLCTxReset;
    memcpy(&data.no_param, p_no_param, sizeof(T_Null_Record));
    if (((mqd_t)-1) != q) {
        write_message_to_queue(q, sizeof(DLCTxReset_TCDATA)-4, &data.no_param, data.tc_id);
    } else {
        return -1;
    }
    return 0;
}
T_bcapp_RI_list ii_DLCTxSDU = i_DLCTxSDU;
typedef struct {
    int tc_id;
    T_UART_DLC_SDU p1;
} DLCTxSDU_TCDATA;
...
int SendTC_DLCTxSDU(void *p_p1)
{
    ...
    DLCTxSDU_TCDATA data;
    data.tc_id = (int) i_DLCTxSDU;
    memcpy(&data.p1, p_p1, sizeof(T_UART_DLC_SDU));
    if (((mqd_t)-1) != q) {
        write_message_to_queue(q, sizeof(DLCTxSDU_TCDATA)-4, &data.p1, data.tc_id);
    } else {
        return -1;
    }
    return 0;
