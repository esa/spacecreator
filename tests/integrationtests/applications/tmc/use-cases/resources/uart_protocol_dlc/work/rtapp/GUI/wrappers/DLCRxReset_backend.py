...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_DLCRxReset
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_rtapp_RI_DLCRxReset
    tcId = i_rtapp_RI_DLCRxReset
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global DLCRxReset_via_shared_lib
    shared_lib = True
    DLCRxReset_via_shared_lib = dll.rtapp_PI_DLCRxReset
...
def send_DLCRxReset_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_DLCRxReset(tc)
            except IOError:
                raise
    elif udp:
