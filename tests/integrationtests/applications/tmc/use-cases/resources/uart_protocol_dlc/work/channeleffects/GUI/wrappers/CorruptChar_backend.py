...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_CorruptChar
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_channeleffects_RI_CorruptChar
    tcId = i_channeleffects_RI_CorruptChar
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global CorruptChar_via_shared_lib
    shared_lib = True
    CorruptChar_via_shared_lib = dll.channeleffects_PI_CorruptChar
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.UINT8_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.UINT8_REQUIRED_BYTES_FOR_ENCODING)
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_CorruptChar(tc)
            except IOError:
                raise
    elif udp:
