...
def setMsgQ():
    global tmId
    tmId = PythonController.i_DLCRxError


def setUDP():
    global tmId
    from _InterfaceEnum import i_rtapp_PI_DLCRxError
    tmId = i_rtapp_PI_DLCRxError


def DLCRxError(tm_ptr, size):
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.T_UART_DLC_RxErrorCode_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def decode_ACN(ACN_encodedBuffer):
    ...
    decoded_value = ASN1.T_UART_DLC_RxErrorCode()
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.T_UART_DLC_RxErrorCode_REQUIRED_BYTES_FOR_ENCODING)
...
def decode_TM(rawTM):
    ...
    tm = ASN1.T_UART_DLC_RxErrorCode()
    tm.SetData(rawTM)
    return tm
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ DLCRxError ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received DLCRxError with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected DLCRxError (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print('Received other (%s), but still waiting for DLCRxError') % str(msgId)
            Q.task_done()


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type T_UART_DLC_RxErrorCode
    # DLCRxError = ASN1.T_UART_DLC_RxErrorCode()
    return None

def typeInstance():
    # Create an intance of an ASN.1 ctype
    return ASN1.T_UART_DLC_RxErrorCode()
