...
typedef struct _RI_Messages {
   T_bcapp_RI_list msg_id;
   union {
      asn1SccT_UART_DLC_SDU dlctxsdu_param;
   } msg_data;
} RI_Messages;


// Queues of messages going from the user to the binary GUI (RIs, or TCs)
static mqd_t bcapp_RI_queue_id;
...
void bcapp_startup(void)
{
   ...
   if (msgsize_max < sizeof (PI_Messages) || msgsize_max < sizeof (RI_Messages)) {
    ...
    len = snprintf (gui_queue_name, 0, "%d_bcapp_RI_queue", geteuid());
    gui_queue_name = (char *) malloc ((size_t) len + 1);
    if (NULL != gui_queue_name) {
       snprintf (gui_queue_name, len + 1, "%d_bcapp_RI_queue", geteuid());

       create_exchange_queue(gui_queue_name, 5, sizeof(RI_Messages), &bcapp_RI_queue_id);
...
void bcapp_PI_Poll(void)
{
   ...
   T_bcapp_RI_list message_received_type;
   ...
   mq_getattr(bcapp_RI_queue_id, &msgq_attr);

   while (!retrieve_message_from_queue(bcapp_RI_queue_id,
                                       sizeof(RI_Messages),
                                       msgcontent,
      ...
      switch(message_received_type) {
         case i_DLCTxReset:
            ...
            break;
         case i_DLCTxSDU:
            bcapp_RI_DLCTxSDU((asn1SccT_UART_DLC_SDU *)msgcontent);
            break;
       default : break;
      }
   }
