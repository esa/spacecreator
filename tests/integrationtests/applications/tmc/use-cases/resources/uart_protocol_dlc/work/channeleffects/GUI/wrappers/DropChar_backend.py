...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_DropChar
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_channeleffects_RI_DropChar
    tcId = i_channeleffects_RI_DropChar
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global DropChar_via_shared_lib
    shared_lib = True
    DropChar_via_shared_lib = dll.channeleffects_PI_DropChar
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_DropChar(tc)
            except IOError:
                raise
    elif udp:
