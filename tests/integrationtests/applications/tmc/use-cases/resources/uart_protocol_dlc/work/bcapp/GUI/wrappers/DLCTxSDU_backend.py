...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_DLCTxSDU
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_bcapp_RI_DLCTxSDU
    tcId = i_bcapp_RI_DLCTxSDU
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global DLCTxSDU_via_shared_lib
    shared_lib = True
    DLCTxSDU_via_shared_lib = dll.bcapp_PI_DLCTxSDU
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_DLCTxSDU(tc)
            except IOError:
                raise
    elif udp:
