...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_DLCTxReset
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_bcapp_RI_DLCTxReset
    tcId = i_bcapp_RI_DLCTxReset
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global DLCTxReset_via_shared_lib
    shared_lib = True
    DLCTxReset_via_shared_lib = dll.bcapp_PI_DLCTxReset
...
def send_DLCTxReset_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_DLCTxReset(tc)
            except IOError:
                raise
    elif udp:
