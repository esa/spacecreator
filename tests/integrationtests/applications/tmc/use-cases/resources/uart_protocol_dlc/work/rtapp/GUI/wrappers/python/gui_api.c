...
T_rtapp_PI_list ii_DLCRxError = i_DLCRxError;
T_rtapp_PI_list ii_DLCRxSDU = i_DLCRxSDU;
T_rtapp_RI_list ii_DLCRxReset = i_DLCRxReset;
typedef struct {
    int tc_id;
...
int SendTC_DLCRxReset(void *p_no_param)
{
    ...
    if (((mqd_t)-2) == q) {
        ...
        sprintf(QName, "%d_rtapp_RI_queue", geteuid());
        open_exchange_queue_for_writing(QName, &q);
    }
    DLCRxReset_TCDATA data;
    data.tc_id = (int) i_DLCRxReset;
    memcpy(&data.no_param, p_no_param, sizeof(T_Null_Record));
    if (((mqd_t)-1) != q) {
        write_message_to_queue(q, sizeof(DLCRxReset_TCDATA)-4, &data.no_param, data.tc_id);
    } else {
        return -1;
    }
    return 0;
