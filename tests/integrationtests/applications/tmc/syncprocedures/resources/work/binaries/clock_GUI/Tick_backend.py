...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_clock_RI_Tick
    tcId = i_clock_RI_Tick
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global Tick_via_shared_lib
    shared_lib = True
    Tick_via_shared_lib = dll.clock_PI_Tick
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_Tick(tc)
            except IOError:
                raise
    elif udp:
