...
package body Pinger_Datamodel with SPARK_Mode is
...
end asn1SccPinger_States_Equal;

function asn1SccPinger_States_Init return asn1SccPinger_States
is
    val: asn1SccPinger_States;
begin
    val := asn1Sccwait;
	pragma Warnings (Off, "object ""val"" is always");
    return val;
	pragma Warnings (On, "object ""val"" is always");
end asn1SccPinger_States_Init;
...
begin
    ...
    ret.ErrorCode := (if ret.Success then 0 else ERR_PINGER_STATES);
    return ret;
end asn1SccPinger_States_IsConstraintValid;
...
begin
    result := asn1SccPinger_States_IsConstraintValid(val);
...
end asn1SccPinger_States_Encode_aux;
...
is
begin
    Stream := asn1SccPinger_States_uPER_Stream'(Size_In_Bytes => 1, Current_Bit_Pos  => 0, pushDataPrm  => 0, fetchDataPrm => 0, Buffer => (others => 0));
    asn1SccPinger_States_Encode_aux(val, Stream, result);
end asn1SccPinger_States_Encode;
...
begin
    ...
    begin
        ...
        if result.Success then
            case asn1SccPinger_States_index_range(intVal) is
                when 0 => val := asn1Sccwait;
            end case;
            result.ErrorCode := 0;
        else
            val := asn1Sccwait;					-- COVERAGE_IGNORE
            result.ErrorCode := ERR_UPER_DECODE_PINGER_STATES;				-- COVERAGE_IGNORE
        end if;
    end;
end asn1SccPinger_States_Decode_aux;
...
begin
    ...
    asn1SccPinger_States_Decode_aux(val, Stream, result);
    if result.success then
        result := asn1SccPinger_States_IsConstraintValid(val);
    end if;

end asn1SccPinger_States_Decode;
...
end asn1SccPinger_Context_Equal;

function asn1SccPinger_Context_Init return asn1SccPinger_Context
is
    val: asn1SccPinger_Context;
begin

    --set state 
    val.state := asn1SccPinger_States_Init;
    --set init_done 
    val.init_done := FALSE;
    --set pongarg 
    ...
    return val;
	pragma Warnings (On, "object ""val"" is always");
end asn1SccPinger_Context_Init;
...
is
    ...
    pragma Warnings (On, "initialization of ret has no effect");        
begin
    ret := asn1SccPinger_States_IsConstraintValid(val.state);
    if ret.Success then
        ret := SYNCPROCEDURES_DATAVIEW.asn1SccMyInteger_IsConstraintValid(val.pongarg);
    end if;
    return ret;
end asn1SccPinger_Context_IsConstraintValid;
...
begin
    result := asn1SccPinger_Context_IsConstraintValid(val);
    if result.success and  result.ErrorCode = 0 then
        --Encode state 
        asn1SccPinger_States_Encode_aux(val.state, bs, result);
        if result.Success then
            --Encode init_done 
            adaasn1rtl.encoding.uper.UPER_Enc_Boolean(bs, val.init_done);
            if result.Success then
                --Encode pongarg 
                SYNCPROCEDURES_DATAVIEW.asn1SccMyInteger_Encode_aux(val.pongarg, bs, result);
            end if;
        end if;
    end if;

end asn1SccPinger_Context_Encode_aux;
...
is
begin
    Stream := asn1SccPinger_Context_uPER_Stream'(Size_In_Bytes => asn1SccPinger_Context_REQUIRED_BYTES_FOR_ENCODING, Current_Bit_Pos  => 0, pushDataPrm  => 0, fetchDataPrm => 0, Buffer => (others => 0));
    asn1SccPinger_Context_Encode_aux(val, Stream, result);
end asn1SccPinger_Context_Encode;
...
begin
    --Decode state 
    asn1SccPinger_States_Decode_aux(val.state, bs, result);
    if result.Success then
        --Decode init_done 
        result.ErrorCode := ERR_UPER_DECODE_PINGER_CONTEXT_INIT_DONE;
        adaasn1rtl.encoding.uper.UPER_Dec_boolean(bs, val.init_done, result.Success);
        if result.Success then
            --Decode pongarg 
            SYNCPROCEDURES_DATAVIEW.asn1SccMyInteger_Decode_aux(val.pongarg, bs, result);
        end if;
    end if;
end asn1SccPinger_Context_Decode_aux;
...
begin
    Stream.Current_Bit_Pos :=0;
    asn1SccPinger_Context_Decode_aux(val, Stream, result);
    if result.success then
        result := asn1SccPinger_Context_IsConstraintValid(val);
    end if;

end asn1SccPinger_Context_Decode;
