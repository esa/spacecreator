...
package body Simulator_Interface is
   ...
   begin
      ...
      Clock_Events.Startup (User_State);
      Pinger_Events.Startup (User_State);
      Ponger_Events.Startup (User_State);
      Update_State (Full_State, User_State);
      ...
      case Event.Kind is
         ...
         when Input_Event_PRESENT =>
            case Event.Input_Event.Event.Kind is
               when Clock_PRESENT  =>
                  Clock_Events.Process_Event  (Event, User_State);
               when Pinger_PRESENT =>
                  Pinger_Events.Process_Event (Event, User_State);
               when Ponger_PRESENT =>
                  Ponger_Events.Process_Event (Event, User_State);
            end case;
   ...
   begin
      ...
      loop
         ...
         exit when Stop_Condition or (Simulator.Events.Length = 0 and User_State.Pinger_Queue.Length = 0 and User_State.Ponger_Queue.Length = 0);
         ...
         -- for (4):
         if not Stop_Condition then
            for I in 1 .. User_State.Pinger_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Pinger_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Pinger_Queue.Length := 0;
             Update_State (Full_State, User_State);
            for I in 1 .. User_State.Ponger_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Ponger_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Ponger_Queue.Length := 0;
             Update_State (Full_State, User_State);
          end if;
      end loop;
   ...
   begin
      ...
      ES.Backup_Ctxt := Full_State;
      ES.Backup_Hash := ES.State_Hash (Full_State);
      Clock_Events.Exhaust_Interfaces (User_State, Callback => ES_Callback'Access);
   ...
   begin
      case Event.Kind is
         ...
         when Input_Event_PRESENT =>
            case Event.Input_Event.Event.Kind is
               when Clock_PRESENT                  =>
                  Clock_Events.Print_Event (Event  => Event.Input_Event.Event.Clock);
               when Pinger_PRESENT                 =>
                  Pinger_Events.Print_Event (Event => Event.Input_Event.Event.Pinger);
               when Ponger_PRESENT                 =>
                  Ponger_Events.Print_Event (Event => Event.Input_Event.Event.Ponger);
            end case;
         when Output_Event_PRESENT =>
            case Event.Output_Event.Event.Kind is
               when Clock_PRESENT                  =>
                  Clock_Events.Print_Event (Event  => Event.Output_Event.Event.Clock);
               when Pinger_PRESENT                 =>
                  Pinger_Events.Print_Event (Event => Event.Output_Event.Event.Pinger);
               when Ponger_PRESENT                 =>
                  Ponger_Events.Print_Event (Event => Event.Output_Event.Event.Ponger);
            end case;
      end case;
   end Print_Event;
   ...
   procedure Pinger_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Pinger_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Pinger_Events.CS_Barrier;
   end Pinger_Check_Queue;

   procedure Ponger_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Ponger_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Ponger_Events.CS_Barrier;
   end Ponger_Check_Queue;
