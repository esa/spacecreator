...
def setMsgQ():
    global tmId
    tmId = PythonController.i_PI_2


def setUDP():
    global tmId
    from _InterfaceEnum import i_function_1_PI_PI_2
    tmId = i_function_1_PI_PI_2


def PI_2(tm_ptr, size):
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.MyChoice_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.MyChoice_REQUIRED_BYTES_FOR_ENCODING)
    ...
    while True:
        ...
        if msgId == tmId:
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received PI-2 with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected PI-2 (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print('Received other (%s), but still waiting for PI-2') % str(msgId)
            Q.task_done()


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type MyChoice
    # PI_2 = ASN1.MyChoice()
    return None

def typeInstance():
    # Create an intance of an ASN.1 ctype
    return ASN1.MyChoice()
