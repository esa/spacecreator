...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_function_1_RI_PI_1
    tcId = i_function_1_RI_PI_1
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global PI_1_via_shared_lib
    shared_lib = True
    PI_1_via_shared_lib = dll.function_1_PI_PI_1
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.MySeqOf_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.MySeqOf_REQUIRED_BYTES_FOR_ENCODING)
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_PI_1(tc)
            except IOError:
                raise
    elif udp:
...
def fromPysideToASN1(_):
    ...
    # Create a native ASN.1 variable of type MySeqOf
    # PI_1 = ASN1.MySeqOf()
    return None
