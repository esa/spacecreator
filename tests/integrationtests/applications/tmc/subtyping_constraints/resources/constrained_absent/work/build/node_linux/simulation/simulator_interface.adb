...
with Function_1_Events;
with Function_2_Events;
...
package body Simulator_Interface is
   ...
   begin
      ...
      Function_1_Events.Startup (User_State);
      Function_2_Events.Startup (User_State);
      Update_State (Full_State, User_State);
      ...
      case Event.Kind is
         ...
         when Input_Event_PRESENT =>
            case Event.Input_Event.Event.Kind is
               when Function_1_PRESENT =>
                  Function_1_Events.Process_Event (Event, User_State);
               when Function_2_PRESENT =>
                  Function_2_Events.Process_Event (Event, User_State);
            end case;
   ...
   begin
      ...
      loop
         ...
         exit when Stop_Condition or (Simulator.Events.Length = 0 and User_State.Function_2_Queue.Length = 0);
         ...
         -- for (4):
         if not Stop_Condition then
            for I in 1 .. User_State.Function_2_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Function_2_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Function_2_Queue.Length := 0;
             Update_State (Full_State, User_State);
          end if;
      end loop;
   ...
   begin
      ...
      ES.Backup_Ctxt := Full_State;
      ES.Backup_Hash := ES.State_Hash (Full_State);
      Function_1_Events.Exhaust_Interfaces (User_State, Callback => ES_Callback'Access);
...
--              when Function_1, Function_2_PRESENT =>
--                 Function_1, Function_2_Events.Print_Event (Event => Event.Input_Event.Event.Function_1, Function_2);
--              @@END_TABLE@@
--           end case;
--        when Output_Event_PRESENT =>
--           case Event.Output_Event.Event.Kind is
--              @@TABLE'ALIGN_ON("=>", "(", "Global")@@
--              when Function_1, Function_2_PRESENT =>
--                 Function_1, Function_2_Events.Print_Event (Event => Event.Output_Event.Event.Function_1, Function_2);
--              @@END_TABLE@@
--           end case;
--     end case;
   end Print_Event;
   ...
   procedure Function_2_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Function_2_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Function_2_Events.CS_Barrier;
