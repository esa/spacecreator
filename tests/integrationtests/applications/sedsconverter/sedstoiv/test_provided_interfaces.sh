...
DIFF="diff -w -B"
XMLDIFF="xmldiff"
TEST_OUTPUT_DIR=output_test_provided_interfaces
...
# Merge ASN files
sed -e '$s/$/\n/' -s INTERFACES.asn INTERFACES-PROVIDER.asn > dataview-uniq.asn
sed -e '$s/$/\n/' -s INTERFACES.acn INTERFACES-PROVIDER.acn > dataview-uniq.acn
...
$XMLDIFF interfaceview.xml ../resources/test_provided_interfaces.output \
  && $UPDATE_DATAVIEW \
