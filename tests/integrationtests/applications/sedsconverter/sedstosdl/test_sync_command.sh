...
DIFF="diff -w -B"
TEST_OUTPUT_DIR=output_test_sync_command
...
# Translate
$SEDS_CONVERTER --from SEDS --to SDL --aux-models ASN.1,InterfaceView --skip-validation -i resources/test_sync_command.xml \
...
cp resources/test_sync_command.system_structure output/system_structure.pr
cp $TEST_OUTPUT_DIR/SYNCCOMMANDDEMO.asn $TEST_OUTPUT_DIR/dataview-uniq.asn
# Rename the module to avoid naming conflicts
sed -i 's/SYNCCOMMANDDEMO/SYSTEM-DATAVIEW/g' $TEST_OUTPUT_DIR/dataview-uniq.asn
cd $TEST_OUTPUT_DIR
