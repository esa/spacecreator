...
DIFF="diff -w -B"
TEST_OUTPUT_DIR=output_test_failure_reporting_sync
...
# Translate
$SEDS_CONVERTER --from SEDS --to SDL --aux-models ASN.1,InterfaceView --skip-validation -i resources/test_failure_reporting_sync.xml \
...
cp resources/test_failure_reporting_sync_client.system_structure output/test_failure_reporting_sync_client.system_structure
cp resources/test_failure_reporting_sync_calculator.system_structure output/test_failure_reporting_sync_calculator.system_structure
...
$DIFF client.pr ../resources/test_failure_reporting_sync_client.output \
    && $DIFF calculator.pr ../resources/test_failure_reporting_sync_calculator.output \
    && $OPENGEODE --toAda test_failure_reporting_sync_client.system_structure client.pr \
    && asn1scc -Ada --type-prefix asn1Scc -equal dataview-uniq.asn client_datamodel.asn \
    && gcc -c client.adb \
    && $OPENGEODE --toAda test_failure_reporting_sync_calculator.system_structure calculator.pr \
