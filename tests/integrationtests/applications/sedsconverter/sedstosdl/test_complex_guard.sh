...
DIFF="diff -w -B"
TEST_OUTPUT_DIR=output_test_complex_guard
...
# Translate
$SEDS_CONVERTER --from SEDS --to SDL --aux-models ASN.1 --skip-validation -i resources/test_complex_guard.xml \
...
cp resources/test_complex_guard.system_structure output/system_structure.pr
cp $TEST_OUTPUT_DIR/COMPLEXGUARD.asn $TEST_OUTPUT_DIR/dataview-uniq.asn
# Rename the module to avoid naming conflicts
sed -i 's/COMPLEXGUARD/SYSTEM-DATAVIEW/g' $TEST_OUTPUT_DIR/dataview-uniq.asn
cd $TEST_OUTPUT_DIR
# Compare output against reference, and compile to make sure the reference is valid
# Clean (rm) only if all steps pass
$DIFF component.pr ../resources/test_complex_guard.output \
