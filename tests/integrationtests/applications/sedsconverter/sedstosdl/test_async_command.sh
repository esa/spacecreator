...
DIFF="diff -w -B"
TEST_OUTPUT_DIR=output_test_async_command
...
# Translate
$SEDS_CONVERTER --from SEDS --to SDL --aux-models ASN.1,InterfaceView --skip-validation -i resources/test_async_command.xml \
...
cp resources/test_async_command.system_structure output/system_structure.pr
cp $TEST_OUTPUT_DIR/ASYNCCOMMANDDEMO.asn $TEST_OUTPUT_DIR/dataview-uniq.asn
# Rename the module to avoid naming conflicts
sed -i 's/ASYNCCOMMANDDEMO/SYSTEM-DATAVIEW/g' $TEST_OUTPUT_DIR/dataview-uniq.asn
cd $TEST_OUTPUT_DIR
...
$DIFF component.pr ../resources/test_async_command.output \
