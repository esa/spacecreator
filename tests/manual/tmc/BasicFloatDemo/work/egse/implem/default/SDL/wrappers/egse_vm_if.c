...
void egse_tm
      (const char *IN_y, size_t IN_y_len)

{
   sender_pid = egse_tm_get_sender();
   ...
   // Call user code
   egse_PI_tm ((asn1SccT_Int *)IN_y);

   // trace the SDL state (appears on the MSC)
   if (1 == innerMsc) {
      extern char *egse_state(void);
      printf ("INNER_SDL_STATE: egse,%s\n", egse_state());
      fflush(stdout);
   }
}
