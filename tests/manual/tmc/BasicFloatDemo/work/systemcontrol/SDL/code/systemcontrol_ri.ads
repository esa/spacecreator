...
package Systemcontrol_RI is

   procedure Controlsignal (X : in out asn1SccT_Float; Dest_PID : asn1sccPID := asn1sccEnv);
   
   
   procedure Tm (Y : in out asn1SccT_Int; Dest_PID : asn1sccPID := asn1sccEnv);

   procedure Check_Queue (Res : out adaasn1rtl.Asn1Boolean)
      with Import, Convention => C, Link_Name => "systemcontrol_check_queue";

   procedure Get_Sender (Sender : out asn1sccPID; Dest_PID : asn1SccPID := asn1sccEnv)
      with Import, Convention => C, Link_Name => "systemcontrol_get_sender";
