...
package body Egse_Events is
      ...
      case Evt.Kind is
         when Msg_IN_Present =>
            case Evt.Msg_In.Kind is
               when Tm_PRESENT =>
                  declare
                     Param : asn1SccT_Int := Evt.Msg_In.Tm.Y;
                  begin
                     Egse_PI.Tm (Y => Param, Global_State => Global_State, Sender_PID => Event.Input_Event.Source);
                  end;
            end case;
                                ...
                                Systemcontrol =>
                                   (Kind   => Msg_In_PRESENT,
                                    Msg_In => (Kind => Tc_PRESENT,
                                               Tc =>
                                                  (x =>  Event.Output_Event.Event.Egse.Msg_Out.Tc.x))));
      ...
      case Event.Kind is
         when Msg_IN_Present =>
            case Event.Msg_In.Kind is
               when Tm_PRESENT =>
                 Put ("EGSE: INPUT Tm");
                  Put_Line (" (" & T_Int_Pkg.Image (Event.Msg_In.Tm.Y) & ")");
            end case;
         when Msg_OUT_Present =>
            case Event.Msg_Out.Kind is
                when Tc_PRESENT =>
                 Put ("EGSE: OUTPUT Tc");
                  Put_Line (" (" & T_Int_Pkg.Image (Event.Msg_Out.Tc.X) & ")");
            end case;
      end case;
   end Print_Event;
   ...
   begin
      ...
      Event.Output_Event.Dest   := asn1SccSystemcontrol;
      ...
      for Each of Iterator loop
         ...
         Param := T_Int_Pkg.To_ASN1 (Each);
         Event.Output_Event.Event.Egse.Msg_Out.Tc.x := Param;
