         ...
         when Msg_IN_Present =>
            case Evt.Msg_In.Kind is
               when Input_None_PRESENT =>
                  ...
                  CS_Barrier := True;
               when Trigger_PRESENT =>
                  Sensor_PI.Trigger (Global_State => Global_State, Sender_PID => Event.Input_Event.Source);
            end case;
            ...
            case Event.Msg_Out.Kind is
                when Reading_PRESENT =>
                 Put ("Sensor: OUTPUT Reading");
                  Put_Line (" (" & T_Float_Pkg.Image (Event.Msg_Out.Reading.X) & ")");
            end case;
      end case;
   end Print_Event;
