...
package body Simulator_Interface is
   ...
   begin
      ...
      Systemcontrol_Events.Startup (User_State);
      Update_State (Full_State, User_State);
      ...
      case Event.Kind is
         ...
         when Input_Event_PRESENT =>
            case Event.Input_Event.Event.Kind is
               ...
               when Systemcontrol_PRESENT =>
                  Systemcontrol_Events.Process_Event (Event, User_State);
            end case;
   ...
   begin
      ...
      loop
         ...
         exit when Stop_Condition or (Simulator.Events.Length = 0 and User_State.Actuator_Queue.Length = 0 and User_State.Sensor_Queue.Length = 0 and User_State.Systemcontrol_Queue.Length = 0);
         ...
         if not Stop_Condition then
            ...
            for I in 1 .. User_State.Sensor_Queue.Length loop
             ...
             Update_State (Full_State, User_State);
            for I in 1 .. User_State.Systemcontrol_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Systemcontrol_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Systemcontrol_Queue.Length := 0;
             Update_State (Full_State, User_State);
          end if;
      end loop;
...
--              when Actuator, Egse, Sensor, Systemcontrol_PRESENT =>
--                 Actuator, Egse, Sensor, Systemcontrol_Events.Print_Event (Event => Event.Input_Event.Event.Actuator, Egse, Sensor, Systemcontrol);
--              @@END_TABLE@@
--           end case;
--        when Output_Event_PRESENT =>
--           case Event.Output_Event.Event.Kind is
--              @@TABLE'ALIGN_ON("=>", "(", "Global")@@
--              when Actuator, Egse, Sensor, Systemcontrol_PRESENT =>
--                 Actuator, Egse, Sensor, Systemcontrol_Events.Print_Event (Event => Event.Output_Event.Event.Actuator, Egse, Sensor, Systemcontrol);
--              @@END_TABLE@@
--           end case;
--     end case;
   end Print_Event;
   ...
   procedure Systemcontrol_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Systemcontrol_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Systemcontrol_Events.CS_Barrier;
   end Systemcontrol_Check_Queue;
