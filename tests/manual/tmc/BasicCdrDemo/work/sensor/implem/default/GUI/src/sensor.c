...
static mqd_t sensor_RI_queue_id;
...
void sensor_startup(void)
{
   ...
   if (msgsize_max < sizeof (PI_Messages) || msgsize_max < sizeof (RI_Messages)) {
    ...
    len = snprintf (gui_queue_name, 0, "%d_sensor_RI_queue", geteuid());
    gui_queue_name = (char *) malloc ((size_t) len + 1);
    if (NULL != gui_queue_name) {
       snprintf (gui_queue_name, len + 1, "%d_sensor_RI_queue", geteuid());

       create_exchange_queue(gui_queue_name, 5, sizeof(RI_Messages), &sensor_RI_queue_id);
...
void sensor_PI_Poll(void)
{
   ...
   T_sensor_RI_list message_received_type;
   ...
   mq_getattr(sensor_RI_queue_id, &msgq_attr);

   while (!retrieve_message_from_queue(sensor_RI_queue_id,
                                       sizeof(RI_Messages),
                                       msgcontent,
