...
typedef struct _PI_Messages {
   ...
   union {
      asn1SccSystemTelemetry systemtm_param;
   } msg_data;
} PI_Messages;
...
typedef struct _RI_Messages {
   ...
   union {
      asn1SccSystemCommand systemtc_param;
   } msg_data;
} RI_Messages;
...
void egse_PI_Poll(void)
{
   ...
   while (!retrieve_message_from_queue(egse_RI_queue_id,
      ...
      switch(message_received_type) {
         case i_systemTc:
            egse_RI_systemTc((asn1SccSystemCommand *)msgcontent);
            break;
       default : break;
      }
   }
