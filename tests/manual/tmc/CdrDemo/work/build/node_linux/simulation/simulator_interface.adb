...
with Actuator1_Events;
with Actuator2_Events;
with Egse_Events;
with Modemanager_Events;
with Sensor_Events;
...
package body Simulator_Interface is
   ...
   begin
      ...
      Actuator1_Events.Startup (User_State);
      Actuator2_Events.Startup (User_State);
      Egse_Events.Startup (User_State);
      Modemanager_Events.Startup (User_State);
      Sensor_Events.Startup (User_State);
      Update_State (Full_State, User_State);
      ...
      case Event.Kind is
         ...
         when Input_Event_PRESENT =>
            case Event.Input_Event.Event.Kind is
               when Actuator1_PRESENT   =>
                  Actuator1_Events.Process_Event   (Event, User_State);
               when Actuator2_PRESENT   =>
                  Actuator2_Events.Process_Event   (Event, User_State);
               when Egse_PRESENT        =>
                  Egse_Events.Process_Event        (Event, User_State);
               when Modemanager_PRESENT =>
                  Modemanager_Events.Process_Event (Event, User_State);
               when Sensor_PRESENT      =>
                  Sensor_Events.Process_Event      (Event, User_State);
            end case;
   ...
   begin
      ...
      loop
         ...
         exit when Stop_Condition or (Simulator.Events.Length = 0 and User_State.Actuator1_Queue.Length = 0 and User_State.Actuator2_Queue.Length = 0 and User_State.Modemanager_Queue.Length = 0 and User_State.Sensor_Queue.Length = 0);
         ...
         -- for (4):
         if not Stop_Condition then
            for I in 1 .. User_State.Actuator1_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Actuator1_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Actuator1_Queue.Length := 0;
             Update_State (Full_State, User_State);
            for I in 1 .. User_State.Actuator2_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Actuator2_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Actuator2_Queue.Length := 0;
             Update_State (Full_State, User_State);
            for I in 1 .. User_State.Modemanager_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Modemanager_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Modemanager_Queue.Length := 0;
             Update_State (Full_State, User_State);
            for I in 1 .. User_State.Sensor_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Sensor_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Sensor_Queue.Length := 0;
             Update_State (Full_State, User_State);
          end if;
      end loop;
   ...
   begin
      ...
      ES.Backup_Ctxt := Full_State;
      ES.Backup_Hash := ES.State_Hash (Full_State);
      Egse_Events.Exhaust_Interfaces (User_State, Callback => ES_Callback'Access);
...
--              when Actuator1, Actuator2, Egse, Modemanager, Sensor_PRESENT =>
--                 Actuator1, Actuator2, Egse, Modemanager, Sensor_Events.Print_Event (Event => Event.Input_Event.Event.Actuator1, Actuator2, Egse, Modemanager, Sensor);
--              @@END_TABLE@@
--           end case;
--        when Output_Event_PRESENT =>
--           case Event.Output_Event.Event.Kind is
--              @@TABLE'ALIGN_ON("=>", "(", "Global")@@
--              when Actuator1, Actuator2, Egse, Modemanager, Sensor_PRESENT =>
--                 Actuator1, Actuator2, Egse, Modemanager, Sensor_Events.Print_Event (Event => Event.Output_Event.Event.Actuator1, Actuator2, Egse, Modemanager, Sensor);
--              @@END_TABLE@@
--           end case;
--     end case;
   end Print_Event;
   ...
   procedure Actuator1_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Actuator1_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Actuator1_Events.CS_Barrier;
   end Actuator1_Check_Queue;

   procedure Actuator2_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Actuator2_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Actuator2_Events.CS_Barrier;
   end Actuator2_Check_Queue;


   procedure Modemanager_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Modemanager_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Modemanager_Events.CS_Barrier;
   end Modemanager_Check_Queue;

   procedure Sensor_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Sensor_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Sensor_Events.CS_Barrier;
   end Sensor_Check_Queue;
