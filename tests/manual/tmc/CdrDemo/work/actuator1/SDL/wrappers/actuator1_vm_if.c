/* Body file for function Actuator1
...
void actuator1_get_sender(asn1SccPID *OUT_sender_pid, asn1SccPID unused)
{
...
unsigned actuator1_initialized = 0;
void init_actuator1(void)
{
   if (0 == actuator1_initialized) {
      // Initialize epoch for the MSC tracing in debug mode
      long long unused = getTimeInMilliseconds();
      actuator1_initialized = 1;
      // Initialize Ada runtime to call elaboration functions
      adainit();
      puts ("[TASTE] Initialization completed for function Actuator1");
      actuator1_initialized = 2;
   }
}
void actuator1_actuatorTc
      (const char *IN_tc, size_t IN_tc_len)

{
   sender_pid = actuator1_actuatortc_get_sender();
   ...
   if (1 == innerMsc) {
      ...
      printf ("INNER_PI: actuator1,actuatortc,%lld\n", msc_time);
      fflush(stdout);
   }

   //  Declare user code function as external (it may not have a .h if it is in Ada)
   extern void actuator1_PI_actuatorTc
   ...
   // Call user code
   actuator1_PI_actuatorTc ((asn1SccActuatorCommand *)IN_tc);

   // trace the SDL state (appears on the MSC)
   if (1 == innerMsc) {
      extern char *actuator1_state(void);
      printf ("INNER_SDL_STATE: actuator1,%s\n", actuator1_state());
      fflush(stdout);
   }
}
