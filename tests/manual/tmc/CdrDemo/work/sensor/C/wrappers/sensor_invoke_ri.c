...
)
{
   ...
   if (1 == innerMsc) {
      ...
      // Log message to ModeManager (corresponding PI: reading)
      printf ("INNER_RI: sensor,modemanager,reading,reading,%lld\n", msc_time);
      fflush(stdout);
   }
   ...
   vm_sensor_reading
     (dest_pid,
      (void *)IN_x, sizeof(asn1SccDataItem));
