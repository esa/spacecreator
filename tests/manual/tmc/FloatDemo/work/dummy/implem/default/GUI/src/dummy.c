...
static mqd_t dummy_RI_queue_id;
...
void dummy_startup(void)
{
   ...
   if (msgsize_max < sizeof (PI_Messages) || msgsize_max < sizeof (RI_Messages)) {
    ...
    len = snprintf (gui_queue_name, 0, "%d_dummy_RI_queue", geteuid());
    gui_queue_name = (char *) malloc ((size_t) len + 1);
    if (NULL != gui_queue_name) {
       snprintf (gui_queue_name, len + 1, "%d_dummy_RI_queue", geteuid());

       create_exchange_queue(gui_queue_name, 5, sizeof(RI_Messages), &dummy_RI_queue_id);
...
void dummy_PI_Poll(void)
{
   ...
   T_dummy_RI_list message_received_type;
   ...
   mq_getattr(dummy_RI_queue_id, &msgq_attr);

   while (!retrieve_message_from_queue(dummy_RI_queue_id,
                                       sizeof(RI_Messages),
                                       msgcontent,
