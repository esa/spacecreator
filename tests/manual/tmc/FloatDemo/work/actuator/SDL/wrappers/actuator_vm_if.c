...
void actuator_controlSignal
      (const char *IN_controlvalue, size_t IN_controlvalue_len)

{
   sender_pid = actuator_controlsignal_get_sender();
   ...
   if (1 == innerMsc) {
      ...
      printf ("INNER_PI: actuator,controlsignal,%lld\n", msc_time);
      fflush(stdout);
   }

   //  Declare user code function as external (it may not have a .h if it is in Ada)
   extern void actuator_PI_controlSignal
      (const asn1SccNumber *);
   ...
   // Call user code
   actuator_PI_controlSignal ((asn1SccNumber *)IN_controlvalue);
