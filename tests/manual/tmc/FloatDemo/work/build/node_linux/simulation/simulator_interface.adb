...
with Actuator_Events;
with Controlsystem_Events;
with Demo_Timer_Manager_Events;
with Host_Events;
with Sensor_Events;
...
package body Simulator_Interface is
   ...
   begin
      ...
      Actuator_Events.Startup (User_State);
      Controlsystem_Events.Startup (User_State);
      Demo_Timer_Manager_Events.Startup (User_State);
      Host_Events.Startup (User_State);
      ...
      case Event.Kind is
         ...
         when Input_Event_PRESENT =>
            case Event.Input_Event.Event.Kind is
               when Actuator_PRESENT      =>
                  Actuator_Events.Process_Event           (Event, User_State);
               when Controlsystem_PRESENT =>
                  Controlsystem_Events.Process_Event      (Event, User_State);
               when Env_PRESENT           =>
                  Demo_Timer_Manager_Events.Process_Event (Event, User_State);
               when Host_PRESENT          =>
                  Host_Events.Process_Event               (Event, User_State);
               when Sensor_PRESENT        =>
   ...
   begin
      ...
      loop
         ...
         exit when Stop_Condition or (Simulator.Events.Length = 0 and User_State.Actuator_Queue.Length = 0 and User_State.Controlsystem_Queue.Length = 0 and User_State.Host_Queue.Length = 0 and User_State.Sensor_Queue.Length = 0);
         ...
         -- for (4):
         if not Stop_Condition then
            for I in 1 .. User_State.Actuator_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Actuator_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Actuator_Queue.Length := 0;
             Update_State (Full_State, User_State);
            for I in 1 .. User_State.Controlsystem_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Controlsystem_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Controlsystem_Queue.Length := 0;
             Update_State (Full_State, User_State);
            for I in 1 .. User_State.Host_Queue.Length loop
               exit when Stop_Condition;
               declare
                  New_In_Event : aliased asn1sccObservable_Event :=
                          User_State.Host_Queue.Data (I);
                begin
                   Next_Event (New_In_Event);
                end;
             end loop;
             User_State.Host_Queue.Length := 0;
             Update_State (Full_State, User_State);
...
--              when Actuator, Controlsystem, Demo_Timer_Manager, Host, Sensor_PRESENT =>
--                 Actuator, Controlsystem, Demo_Timer_Manager, Host, Sensor_Events.Print_Event (Event => Event.Input_Event.Event.Actuator, Controlsystem, Demo_Timer_Manager, Host, Sensor);
--              @@END_TABLE@@
--           end case;
--        when Output_Event_PRESENT =>
--           case Event.Output_Event.Event.Kind is
--              @@TABLE'ALIGN_ON("=>", "(", "Global")@@
--              when Actuator, Controlsystem, Demo_Timer_Manager, Host, Sensor_PRESENT =>
--                 Actuator, Controlsystem, Demo_Timer_Manager, Host, Sensor_Events.Print_Event (Event => Event.Output_Event.Event.Actuator, Controlsystem, Demo_Timer_Manager, Host, Sensor);
--              @@END_TABLE@@
--           end case;
--     end case;
   end Print_Event;
   ...
   procedure Actuator_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Actuator_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Actuator_Events.CS_Barrier;
   end Actuator_Check_Queue;

   procedure Controlsystem_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Controlsystem_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Controlsystem_Events.CS_Barrier;
   end Controlsystem_Check_Queue;


   procedure Host_Check_Queue (res : out Boolean) is
   begin
      --  res := User_State.Host_Queue.Length /= 0;
      --  For simulation use a barrier: continuous signals are evaluated
      --  only from the explicit call
      res := Host_Events.CS_Barrier;
   end Host_Check_Queue;
