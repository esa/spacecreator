...
add_block('simulink/Sources/In1', 'Iface/p1');
if all_inports_created
   set_param('Iface/p1','Position', inports_positions(inport_index,:));
else
   set_param('Iface/p1','Position',[25 (inport_index * 100) 55 ((inport_index * 100) + 14)]);
end
set_param('Iface/p1','BusOutputAsStruct','on');
set_param('Iface/p1','OutDataTypeStr','MyInteger');

outport_index = 0;
outport_index = outport_index + 1;
add_block('simulink/Sinks/Out1','Iface/p2');
if all_outports_created
   set_param('Iface/p2','Position', outports_positions(outport_index,:));
else
   set_param('Iface/p2','Position',[430 (outport_index * 100) 460 ((outport_index * 100) + 14)]);
end
set_param('Iface/p2','OutDataTypeStr','MyInteger');
