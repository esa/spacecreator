...
inport_index = inport_index + 1;
add_block('simulink/Sources/In1', 'Iface/output');
if all_inports_created
   set_param('Iface/output','Position', inports_positions(inport_index,:));
else
   set_param('Iface/output','Position',[25 (inport_index * 100) 55 ((inport_index * 100) + 14)]);
end
set_param('Iface/output','BusOutputAsStruct','on');
set_param('Iface/output','UseBusObject','on');
set_param('Iface/output','BusObject','MySeq');
add_block('simulink/Commonly Used Blocks/Bus Selector','Iface/output_MySeq_BusSel');
add_line('Iface','output/1','output_MySeq_BusSel/1');
setOutputsBusSelector(MySeq, 'Iface/output_MySeq_BusSel');
if all_inports_created
   set_param('Iface/output_MySeq_BusSel','Position', bussel_positions(inport_index,:));
else
   set_param('Iface/output_MySeq_BusSel','Position',[95 ((inport_index * 100) - 19) 100 ((inport_index * 100) + 19)]);
end
